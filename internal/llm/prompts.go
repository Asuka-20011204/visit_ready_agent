...
4. 相同事实只能出现一次；事实只需输出 category 与 source_quote，content 由系统使用原文补齐。一个长句包含多个症状时，可以分别建立症状画像，但不得把同一长句复制为多条事实。事实要从原文和各轮追问回答中一起提取，包括用药、过敏、既往史、检查与测量；用户明确否认的也要作为事实（content 用原文，如“没有糖尿病”“没吃止痛药”）。
