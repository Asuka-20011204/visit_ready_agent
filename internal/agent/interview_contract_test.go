...
func TestDeterministicPromptsPassSafetyContract(t *testing.T) {
	...
	for _, tc := range []struct{ field, category string }{
		...
		t.Run(tc.category, func(t *testing.T) {
			...
			if !isSafeClarificationPrompt(p) {
				t.Fatalf("generated prompt rejected: %+v", p)
			}
		})
	}
}
...
func TestConflictsRespectInterviewBudget(t *testing.T) {
	...
	if len(next.Session.Contradictions) == 0 {
		t.Fatal("fixture did not create conflict")
	}
	if next.Session.Status == domain.StatusWaitingClarification {
		t.Fatal("conflict exceeded interview budget")
	}
}
