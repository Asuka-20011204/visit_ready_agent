...
> 当前推荐部署 `v1.0.4`。安装脚本从 GitHub `main` 下载，镜像和 Compose 仍钉在发布摘要中的提交与 digest。`v1.0.4` 修复第二轮补充输入框被隐藏，以及首次 live 误走 MySQL socket dump。`v1.0.2` 是首个成功发布新版一键部署镜像的版本；`v1.0.0` 不包含这些脚本，`v1.0.1` 发布失败。任何已发布标签都不应移动或覆盖。
